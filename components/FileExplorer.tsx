...
import { FileNode, NodeType } from '../types.ts';
import { FolderIcon, FileIcon, ChevronDownIcon, ThreeDotsIcon, DownloadIcon, NewFileIcon, NewFolderIcon } from './icons.tsx';
...
const FileEntry: React.FC<{ node: FileNode; onFileSelect: (id: string) => void; activeFileId: string | null; level?: number }> = ({ node, onFileSelect, activeFileId, level = 0 }) => {
  ...
  return (
    <div>
      ...
      >
        {isFolder ? (
            <span onClick={toggleFolder} className="transition-transform duration-200" style={{ transform: isOpen ? 'rotate(0deg)' : 'rotate(-90deg)'}}>
                <ChevronDownIcon />
            </span>
...
const FileExplorer: React.FC<FileExplorerProps> = ({ files, onFileSelect, activeFileId, onDownloadProject, onAddItem }) => {
  return (
    <div className="w-64 h-full bg-surface/50 backdrop-blur-md border-r border-white/10 flex flex-col p-2 z-10">
      <div className="flex items-center justify-between p-2 mb-2">
        ...
        <div className="flex items-center space-x-1">
             <button onClick={() => onAddItem('file')} title="New File" className="p-1 rounded-md hover:bg-white/10 text-light-gray hover:text-soft-white">
                <NewFileIcon />
            </button>
            <button onClick={() => onAddItem('folder')} title="New Folder" className="p-1 rounded-md hover:bg-white/10 text-light-gray hover:text-soft-white">
                <NewFolderIcon />
            </button>
